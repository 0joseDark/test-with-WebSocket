...
async def connect_to_server():
    ...
    try:
        ...
        asyncio.create_task(receive_messages())  # Inicia a recepção de mensagens sem bloquear
...
def exit_application():
    if websocket_connection:
        asyncio.run(disconnect_from_server())
    root.quit()
...
# Função para iniciar o loop de eventos do asyncio e a conexão ao servidor
def connect_to_server_wrapper():
    asyncio.run(connect_to_server())

# Função para exibir a mensagem recebida na interface gráfica
...
def create_client_window():
    ...
    global recipient_entry, message_send_button, friend_entry, friendship_var, friend_button, received_messages_text
    ...
    connect_button = tk.Button(root, text="Ligar ao Servidor", command=connect_to_server_wrapper)
    connect_button.grid(row=4, column=0)

    disconnect_button = tk.Button(root, text="Desligar do Servidor", state=tk.DISABLED, command=lambda: asyncio.run(disconnect_from_server()))
    disconnect_button.grid(row=4, column=1)

    send_button = tk.Button(root, text="Login", state=tk.DISABLED, command=lambda: asyncio.run(login()))
    ...
    message_send_button = tk.Button(root, text="Enviar Mensagem", state=tk.DISABLED, command=lambda: asyncio.run(send_user_message()))
    ...
    friend_button = tk.Button(root, text="Definir Status", state=tk.DISABLED, command=lambda: asyncio.run(set_friendship_status()))
    friend_button.grid(row=11, column=0, columnspan=2)

    tk.Label(root, text="Mensagens Recebidas:").grid(row=12, column=0, columnspan=2)
    received_messages_text = tk.Text(root, state=tk.DISABLED, height=10, width=50)
    received_messages_text.grid(row=13, column=0, columnspan=2)

    exit_button = tk.Button(root, text="Sair", command=exit_application)
    exit_button.grid(row=14, column=0, columnspan=2)
